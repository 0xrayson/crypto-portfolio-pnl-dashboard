...
import { getSwapPnl } from "@/lib/server/portfolio/getSwapPnl";

// Refreshes transactions (5 chains + CoinGecko) before computing PnL; a cold
// serverless instance can exceed Vercel's default execution limit.
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ ecosystem: string; address: string }> }) {
  ...
  try {
    ...
    const summary = await getSwapPnl(ecosystem, address);
